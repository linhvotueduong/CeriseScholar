...
- [x] Migration **`015_profiles.sql`** (numbered 015 to fit the consolidated sequence): `profiles` table (id = auth user id; first/last/full name, avatar_url, bio, institution, field_of_study, onboarding_completed, timestamps) + RLS (owner-only select/insert/update) + `handle_new_user()` trigger that auto-creates a profile on every new signup. **✅ DONE 2026-06-14, applied to live + verified.**
- [x] Backfill existing users into `profiles` — **✅ DONE**: 9/9 existing users backfilled from their auth metadata (metadata kept as fallback). Auto-create trigger confirmed installed.
- [x] Write path: signup/complete-profile and Settings → Account save to `profiles`. **✅ DONE 2026-06-14 (code wiring).** New `src/lib/profile/profile.ts` (fetch/upsert + resolve helpers) and `src/hooks/useProfile.ts`. Settings → Account now persists Full name + About to `profiles` (save-on-blur, mirrored into auth metadata). `/auth/complete-profile` upserts identity into `profiles` on both the email-confirm and Google paths.
- [x] Read path: sidebar, top nav, and account pages read from `profiles` (fallback to metadata). **✅ DONE 2026-06-14.** `AppSidebar`, `AppTopNav`, `dashboard/account`, and `settings/account` all read identity through `useProfile` (profile row first, auth metadata fallback). No layout changes.
- [x] **Password reset flow**: "Forgot password?" link → email → reset page. **✅ DONE 2026-06-14.** Added the link to `LoginForm` (mobile + desktop), new `/forgot-password` (request email) and `/auth/reset-password` (PKCE code-exchange → set new password) pages. The dead "Change Password" button now emails the logged-in user a reset link (reuses the same flow; no new modal).
- [x] Fix the Settings "About" textarea that currently saves nothing. **✅ DONE 2026-06-14** — now bound to `profiles.bio` and persisted on blur with an inline "All changes saved." status.
- [x] Add `/auth/complete-profile` to protected routes. **✅ DONE 2026-06-14** — added to `middleware.ts` matcher + protected-path check (logged-out visitors are sent to `/login`); `/auth/callback` and `/auth/reset-password` deliberately left open (they run before a session exists).

**Done when:** a brand-new signup produces a `profiles` row automatically; editing your name in Settings persists and shows in the sidebar; password reset works on a test account. — **Code complete & verified at lint/type/render level (2026-06-14).** Remaining for full sign-off: a live click-through on a real test account (covered by Phase 6's walkthroughs). ✅ ESLint + `tsc --noEmit` clean; `/login`, `/forgot-password`, `/auth/reset-password` return HTTP 200 on the dev server.
...
- [x] Claude drafts `docs/dashboard-metric-contract.md`: for each of the 13 cards — plain-language meaning, exact formula, source tables, empty-account behavior. **✅ DRAFT DONE 2026-06-15.** Every card documented from the real formulas in `deriveDashboardState.ts`; hardcoded component fallbacks flagged per card; 7 decisions surfaced with a recommendation each.
- [ ] You approve/adjust each definition (e.g., "Weekly Activity = your actions this week, where uploading a source counts 4× a click" — keep or change?). **⏳ Awaiting your review of the draft (the 7 decisions in §"Summary of decisions for you").**
- [ ] Decide the open product questions: §8 below. **→ folded into the draft's Decisions 1–7 (activity weights, Today's Target redefinition, Today's Tasks role, weekly bar, blend weights, Cerise Support, demo mixing).**
- [ ] Wire `dashboard_project_settings` into the contract (daily minutes + target date feed "Today's Target"). **→ proposed in the draft as Decision 2 (pending your approval).**

**Done when:** every card has one approved sentence defining it, and the contract file is saved. — **Draft saved (`docs/dashboard-metric-contract.md`); needs your sign-off on the 7 decisions to close the phase.**
...
- **2026-06-14 (cont.5)** — Phase 2 schema done. Created `015_profiles.sql` (profiles table + owner-only RLS + `handle_new_user` trigger), applied to live via dashboard SQL. Verified: table created, **9/9** existing users backfilled, auto-create trigger installed. Remaining Phase 2 = code work (read/write wiring, password reset, fix dead About / Change-Password controls, gate complete-profile).
...
- **2026-06-16** — **Dashboard v2 master spec adopted into the repo + technical appendix written (docs only, no code).** Copied the buildable v2 spec to `docs/dashboard-master-functional-spec-v2.md`; wrote `docs/dashboard-technical-appendix.md` translating it into exact TS types (`CardState<T>`, `DashboardSnapshot`, per-card payloads), the `DashboardSourceData → DashboardSnapshot → UI` pipeline, additive Supabase drafts (migration **016** adds pace/work_weekdays/skipped_dates/manual-override to `dashboard_project_settings`; deferred `dashboard_tasks` recommendation fields + `project_ai_signals`), and an 8-step phased build (A foundation types → B persist Today's Target → C schedule engine → D section checks → E research focus → F activity/greeting → G continue-learning → H AI signals). Confirmed current state: Codex's dashboard UI redesign + `targetPace.ts` are in the (dirty, uncommitted) worktree; `dashboard_project_settings` is still wired nowhere; no `CardState`/snapshot/recommendation-engine yet. Implementation starts at appendix Phase A only after owner approval. See also the metric contract.
- **2026-06-15** — **Phase 3 draft created.** Wrote `docs/dashboard-metric-contract.md` from the real `deriveDashboardState.ts` formulas: all 13 cards (plain meaning + exact formula + source tables + empty-account behavior), the shared "activity points" weight table and 6-stage "section progress" model, every hardcoded component fallback flagged, and 7 product decisions each with a recommendation. No code changed. **Awaiting user sign-off on the 7 decisions to close Phase 3** (then Phase 4 = sample-data badge, Phase 5 = card wiring).
- **2026-06-14 (cont.6)** — **Phase 2 code wiring complete (no live DB changes).** Built the profile data layer (`src/lib/profile/profile.ts`, `src/hooks/useProfile.ts`); rewired read path (`AppSidebar`, `AppTopNav`, `dashboard/account`, `settings/account`) and write path (Settings save-on-blur → `profiles` + metadata mirror; `complete-profile` upsert on both signup paths). Built password reset: "Forgot password?" link in `LoginForm`, new `/forgot-password` + `/auth/reset-password` pages (PKCE code-exchange), and wired the dead "Change Password" button to email a reset link. Protected `/auth/complete-profile` in middleware. ESLint + `tsc --noEmit` clean; `/login`, `/forgot-password`, `/auth/reset-password` return HTTP 200 on the running dev server. **Not yet committed** — awaiting user smoke-test + checkpoint approval (recommended tag `v1.1.0`). Files touched: 7 modified + 6 new (see PROJECT_MAP). **Phase 2 done pending the live-account walkthrough in Phase 6.**
