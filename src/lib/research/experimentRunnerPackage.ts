...
(function(){
  ...
  function checkpoint(status){checkpointSequence+=1;var trials=status==="withdrawn"?[]:trialResults();var record={checkpointVersion:4,checkpointSequence:checkpointSequence,idempotencyKey:sessionId+":"+String(checkpointSequence),releaseId:String(release.releaseId||"unreleased"),releaseNumber:Number(release.releaseNumber)||0,releaseChecksum:String(release.checksum||"unreleased"),sessionId:sessionId,status:status,currentIndex:currentIndex,condition:condition,responses:responses,audioResponses:status==="withdrawn"?{}:audioResponses,videoResponses:status==="withdrawn"?{}:videoResponses,timings:timings,events:events,history:history,trials:trials,trialOrder:trials.map(function(trial){return trial.loopBlockId+":"+trial.trialId+":"+trial.repetition;}),startedAt:studyStartedAt,updatedAt:new Date().toISOString(),executionMode:String(spec.executionMode||"pilot")};dirty=status!=="completed"&&status!=="withdrawn";void writeRecovery(record);void transportCheckpoint(record);return record;}
