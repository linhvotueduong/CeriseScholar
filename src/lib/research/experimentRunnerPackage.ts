...
export const EXPERIMENT_RUNNER_PACKAGE_VERSION = 4 as const;
...
export function collectExperimentPackageChecks(
  ...
  const hasConsent = document.blocks.some((block) => block.type === "consent");
...
export function canBuildExperimentRunnerPackage(document: ExperimentStudioDocument): boolean {
  return document.blocks.length > 0
    && !validateExperimentStudio(document).some((issue) => issue.severity === "error");
}

const RUNNER_STYLE = `
:root{color-scheme:light;--ink:#17130f;--muted:#6d6963;--border:#dedbd5;--gold:#a87f4f;--blue:#eef6fb;--danger:#a72a25;font-family:Arial,Helvetica,sans-serif}*{box-sizing:border-box}body{background:var(--blue);color:var(--ink);margin:0;min-height:100vh}.runner{display:grid;grid-template-rows:58px minmax(0,1fr);min-height:100vh}.runner-header{align-items:center;background:#fff;border-bottom:1px solid var(--border);display:flex;justify-content:space-between;padding:0 24px}.runner-header strong{font-family:Georgia,serif;font-size:20px}.runner-header span{color:var(--muted);font-size:12px}.screen-shell{align-items:center;display:flex;justify-content:center;padding:34px 20px}.screen{background:#fff;border:1px solid var(--border);border-radius:10px;box-shadow:0 12px 34px rgba(42,36,30,.06);max-width:760px;min-height:460px;padding:52px 58px;width:100%}.progress{color:var(--muted);font-size:11px;margin-bottom:34px;text-transform:uppercase}.screen h1{font-family:Georgia,serif;font-size:32px;line-height:1.2;margin:0 0 18px}.stimulus-media{border:1px solid var(--border);border-radius:8px;display:block;height:auto;margin:0 0 24px;max-height:440px;object-fit:contain;width:100%}.prompt{font-size:16px;line-height:1.65;margin:0 0 30px;white-space:pre-wrap}.response{border:0;display:grid;gap:12px;margin:0;padding:0}.response legend{font-size:13px;font-weight:700;margin-bottom:10px}.choice{align-items:center;border:1px solid var(--border);border-radius:7px;cursor:pointer;display:flex;gap:11px;padding:12px 14px}.choice:focus-within{border-color:var(--gold);box-shadow:0 0 0 2px rgba(168,127,79,.14)}.scale{display:flex;flex-wrap:wrap;gap:10px}.scale .choice{display:grid;min-width:54px;text-align:center}.scale-labels{color:var(--muted);display:flex;font-size:11px;justify-content:space-between;margin-bottom:9px}.keyboard-response{background:#f8f6f2;border:1px solid var(--border);border-radius:7px;display:grid;gap:8px;padding:18px;text-align:center}.keyboard-response span{color:var(--muted);font-size:12px}.text-response{border:1px solid var(--border);border-radius:7px;font:inherit;min-height:130px;padding:13px;resize:vertical;width:100%}.navigation{align-items:center;border-top:1px solid var(--border);display:flex;justify-content:space-between;margin-top:40px;padding-top:20px}.button{align-items:center;background:#fff;border:1px solid #bdb8b0;border-radius:7px;color:var(--ink);cursor:pointer;display:inline-flex;font-size:13px;font-weight:700;justify-content:center;min-height:40px;padding:0 17px}.button.primary{background:#231a10;border-color:#231a10;color:#fff}.button:disabled{cursor:not-allowed;opacity:.42}.timing{color:var(--muted);font-size:11px}.completion{align-items:flex-start;display:flex;flex-direction:column;gap:14px}.completion h1,.completion p{margin:0}.downloads{display:flex;flex-wrap:wrap;gap:10px;margin-top:12px}.privacy-note{background:#f8f6f2;border:1px solid var(--border);font-size:12px;line-height:1.5;padding:13px}.ended{color:var(--danger)}@media(max-width:620px){.runner-header{padding:0 14px}.runner-header strong{font-size:17px}.screen-shell{padding:12px}.screen{min-height:calc(100vh - 82px);padding:34px 22px}.screen h1{font-size:26px}.navigation{gap:10px}.runner-header span{display:none}}@media(prefers-reduced-motion:reduce){*{scroll-behavior:auto!important;transition:none!important}}
`;
...
(function(){
  ...
  var responses=Object.create(null),timings=[],history=[],events=[];
  var screenStartedAt=performance.now(),studyStartedAt=new Date().toISOString(),timerIds=[],activeKeyHandler=null;
  var sessionId=createSessionId(),condition=assignCondition(sessionId),runtimeBlocks=materializeBlocks(sessionId,condition),dirty=false,resultDownloaded=false,saveTimer=0,checkpointSequence=0;
  ...
  function clearTimers(){timerIds.forEach(function(id){window.clearTimeout(id);});timerIds=[];if(activeKeyHandler){window.removeEventListener("keydown",activeKeyHandler);activeKeyHandler=null;}}
  ...
  function checkpoint(status){checkpointSequence+=1;var trials=status==="withdrawn"?[]:trialResults();var record={checkpointVersion:2,checkpointSequence:checkpointSequence,idempotencyKey:sessionId+":"+String(checkpointSequence),releaseId:String(release.releaseId||"unreleased"),releaseNumber:Number(release.releaseNumber)||0,releaseChecksum:String(release.checksum||"unreleased"),sessionId:sessionId,status:status,currentIndex:currentIndex,condition:condition,responses:responses,timings:timings,events:events,history:history,trials:trials,trialOrder:trials.map(function(trial){return trial.loopBlockId+":"+trial.trialId+":"+trial.repetition;}),startedAt:studyStartedAt,updatedAt:new Date().toISOString(),executionMode:String(spec.executionMode||"pilot")};dirty=status!=="completed"&&status!=="withdrawn";void writeRecovery(record);void transportCheckpoint(record);return record;}
  ...
  function recordEvent(type){events.push({type:type,at:new Date().toISOString(),blockId:runtimeBlocks[currentIndex]?runtimeBlocks[currentIndex].id:"",screenIndex:currentIndex});if(events.length>500){events=events.slice(-500);}scheduleCheckpoint();}
  ...
  function showResume(saved){root.replaceChildren();var shell=element("main","screen-shell"),card=element("section","screen completion");card.append(element("h1","","Resume this session?"),element("p","","A local recovery checkpoint exists for this exact study release."));var actions=element("div","downloads"),resume=element("button","button primary","Resume"),restart=element("button","button","Start over");resume.type="button";restart.type="button";resume.addEventListener("click",function(){sessionId=String(saved.sessionId||sessionId);responses=saved.responses&&typeof saved.responses==="object"?saved.responses:Object.create(null);timings=Array.isArray(saved.timings)?saved.timings:[];events=Array.isArray(saved.events)?saved.events:[];history=Array.isArray(saved.history)?saved.history:[];studyStartedAt=String(saved.startedAt||studyStartedAt);checkpointSequence=Math.max(0,Number(saved.checkpointSequence)||0);condition=assignCondition(sessionId);runtimeBlocks=materializeBlocks(sessionId,condition);maxVisits=runtimeBlocks.length*4+25;currentIndex=Number.isInteger(saved.currentIndex)?Math.max(0,Math.min(runtimeBlocks.length,saved.currentIndex)):0;render();});restart.addEventListener("click",function(){void deleteRecovery();checkpoint("started");render();});actions.append(resume,restart);card.append(actions);shell.append(card);root.append(shell);}
  window.addEventListener("beforeunload",function(event){if(dirty&&!resultDownloaded){event.preventDefault();event.returnValue="";}});
  ...
  preloadImages().then(function(){return readRecovery();}).then(function(saved){if(saved&&saved.releaseChecksum===String(release.checksum||"unreleased")&&saved.status!=="withdrawn"){showResume(saved);}else{checkpoint("started");render();}});
})();
`;
...
export function buildExperimentRunnerPackage(
  ...
  if (!canBuildExperimentRunnerPackage(document)) {
  ...
  const csp = [
    ...
    "img-src data: blob:",
    options.collectorCheckpointEndpoint ? "connect-src 'self'" : "connect-src 'none'",
    "font-src 'none'",
    "object-src 'none'",
    "base-uri 'none'",
    "form-action 'none'",
  ...
  const runnerScript = RUNNER_SCRIPT.replace("__CHECKPOINT_TRANSPORT__", checkpointTransport);
...
<body>
  <div class="runner">
    <header class="runner-header"><strong>Cerise Scholar</strong><span>${options.collectorCheckpointEndpoint ? "Local Pilot Collector · local network only" : "Portable study runner · no network connection"}</span></header>
