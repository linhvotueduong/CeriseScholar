...
export default function GoogleButton({
  ...
  label?: string;
  onBeforeStart?: (startGoogle: () => void) => boolean | void;
}) {
  ...
  async function handleGoogle() {
    setError(null);
    if (onBeforeStart) {
      const shouldContinue = onBeforeStart(() => {
        void startGoogle();
      });
  ...
  return (
    <div className="w-full">
      <button
        ...
        disabled={loading}
        className="flex min-h-12 w-full items-center justify-center gap-3 rounded-[8px] border border-[#d4cdc5] bg-[#fefefe] px-4 py-3 text-sm font-semibold text-[#1a1208] transition-colors hover:border-[#1a1208] hover:bg-[#faf7f0] disabled:opacity-50"
      >
