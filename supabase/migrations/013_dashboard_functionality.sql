-- ============================================
-- Cerise Scholar — Dashboard Functionality (remote migration 013)
...
--   - policies dropped-then-created (DROP POLICY IF EXISTS ... ; CREATE POLICY ...)
--   - indexes guarded by CREATE INDEX IF NOT EXISTS
-- Additive only: creates new tables; never alters or drops existing data.
-- ============================================
...
DROP POLICY IF EXISTS "Users view own dashboard settings" ON public.dashboard_project_settings;
CREATE POLICY "Users view own dashboard settings"
  ON public.dashboard_project_settings FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users insert own dashboard settings" ON public.dashboard_project_settings;
CREATE POLICY "Users insert own dashboard settings"
  ON public.dashboard_project_settings FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users update own dashboard settings" ON public.dashboard_project_settings;
CREATE POLICY "Users update own dashboard settings"
  ON public.dashboard_project_settings FOR UPDATE
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users delete own dashboard settings" ON public.dashboard_project_settings;
...
DROP POLICY IF EXISTS "Users view own dashboard tasks" ON public.dashboard_tasks;
CREATE POLICY "Users view own dashboard tasks"
  ON public.dashboard_tasks FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users insert own dashboard tasks" ON public.dashboard_tasks;
CREATE POLICY "Users insert own dashboard tasks"
  ON public.dashboard_tasks FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users update own dashboard tasks" ON public.dashboard_tasks;
CREATE POLICY "Users update own dashboard tasks"
  ON public.dashboard_tasks FOR UPDATE
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users delete own dashboard tasks" ON public.dashboard_tasks;
...
DROP POLICY IF EXISTS "Users view own dashboard activity" ON public.dashboard_activity_events;
CREATE POLICY "Users view own dashboard activity"
  ON public.dashboard_activity_events FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users insert own dashboard activity" ON public.dashboard_activity_events;
CREATE POLICY "Users insert own dashboard activity"
  ON public.dashboard_activity_events FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users update own dashboard activity" ON public.dashboard_activity_events;
CREATE POLICY "Users update own dashboard activity"
  ON public.dashboard_activity_events FOR UPDATE
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users delete own dashboard activity" ON public.dashboard_activity_events;
